...
PUBLIC_RECALL_EVIDENCE_GUIDANCE = (
    ...
    "and interpretation without replacing those originals. Retrieval scores "
    "rank bounded candidates, not whether their meaning answers the question. "
    "Interpret the supplied evidence against the current request; background "
    "context does not prove a requested topic or date, and a bounded selection "
    "does not establish absence from the archive. For an ordinary "
    ...
    "Use each original's recorded timestamp for its clock date and time. "
    "Elapsed t+ offsets describe duration; a recurring show schedule is not a recorded start. "
    "Do not estimate a clock time by adding an offset to that schedule. "
    "If the recorded clock timestamp is unavailable, keep that uncertainty explicit. "
