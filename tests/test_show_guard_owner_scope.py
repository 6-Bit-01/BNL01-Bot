...
class ShowGuardOwnerScopeTests(unittest.IsolatedAsyncioTestCase):
    async def test_publication_and_timeline_answer_keeps_publication_clock_and_lore(self):
        ...
        # The old whole-answer check fails despite the supplied publication basis.
        self.assertTrue(bot.finalized_show_packet_owner_requested(request, prompt))
        self.assertTrue(bot.tiktok_show_episode_response_failure(answer, prompt))
    ...
    async def test_dedicated_tiktok_topics_keep_existing_evidence_correction(self):
        ...
        answer = "It was standard baseline chatter and ambient banter."
        repaired = "Green visuals were mentioned by three chatters in three messages."
        with mock.patch.object(
        ...
        self.assertFalse(regenerate.call_args.kwargs["allow_style_rewrite"])
        self.assertEqual(regenerate.call_args.kwargs["route"], "get_gemini_response")
    ...
    async def test_packet_generated_answer_retains_typed_packet_validation_owner(self):
        ...
        with mock.patch.object(
            bot, "tiktok_show_episode_response_failure",
            side_effect=AssertionError("packet answer reached a second show judge"),
        ), mock.patch.object(
            bot, "get_gemini_response_with_optional_typing", new=mock.AsyncMock()
        ) as regenerate:
            result, diagnostics = await bot.apply_guarded_response_regeneration(
