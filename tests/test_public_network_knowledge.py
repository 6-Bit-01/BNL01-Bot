...
class PublicNetworkKnowledgeTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def _batch(self, policy, request=REQUEST, answer=PUBLIC_MEMORY, privileged=True):
        ...
        bnl01_bot._channel_buffers[channel.id].append(("Test Member", request, self.user_id))
        ...
        generation = mock.AsyncMock(return_value=answer)
    ...
    def _seed_publications(self):
        ...
        snapshot = publication_fixtures.control_snapshot(
            # Public visibility is independent of the optional memory-reuse switch.
            memory_excluded=("journal_copper_kite",),
    ...
    async def test_relevant_publications_reach_normal_direct_prompt_guard_and_reply(self):
        self._seed_publications()
        prompt, metadata = await self._direct_prompt_async("public_home", privileged=False)
        ...
        channel = FakeChannel(8820, name="barcode-bot", guild=FakeGuild(self.guild_id))
        message = FakeMessage(channel, REQUEST)
        guarded, diagnostics = await bnl01_bot.apply_guarded_response_regeneration(
            answer,
            prompt=prompt,
            user_id=self.user_id,
            guild_id=self.guild_id,
        ...
        self.assertFalse(diagnostics["source_grounding_guard_triggered"])
    ...
    def test_unchanged_publications_reuse_fresh_snapshot_through_source_checks(self):
        controls = self._seed_publications()
