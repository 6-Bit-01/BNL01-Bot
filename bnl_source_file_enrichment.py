...
QUEUE_NOT_CONNECTED_NOTE = "No confirmed queue or submission history is connected to this Source File yet. Queue/submission data is not connected."
...
def _infer_subject_memory_coverage(packet: dict[str, Any]) -> dict[str, Any]:
    ...
    coverage = [
        ...
        lane("queue/submission data", queue_connected, "Queue/submission data is connected to this report." if queue_connected else "No confirmed queue or submission history is connected to this Source File yet. Queue/submission data is not connected."),
...
def _topic_explanation(topic: str, detail: str, signals: list[str]) -> str:
    ...
    if "music" in lowered or "suno" in lowered or "spotify" in lowered or "soundcloud" in lowered or "youtube" in lowered or "link" in lowered:
        return _case_text(detail or "Music or link sharing appears in the archive, but BNL has not confirmed that those links are the subject's own music. Treat this as a prompt to confirm artist role, link ownership, queue status, and public music links.", 320)
    ...
    if "collab" in lowered or "wip" in lowered or "finished" in lowered:
        return _case_text(detail or "Collaboration or WIP context may help admins map community activity, but BNL still needs to confirm whether this is owned work, helping, sharing, or simply appearing around projects.", 320)
    ...
    return _case_text(detail or f"Extracted evidence points at {topic.lower()}, but the current archive has limited subject-specific detail behind that label.", 320)


def _topic_where_seen(memory: dict[str, Any], topic: str) -> str:
    context = _brief_context(memory)
    channel_names: list[str] = []
    for item in context.get("topChannels") or memory.get("observedChannels") or []:
        if isinstance(item, dict):
            name = _safe_channel_name(item.get("channelName") or item.get("channel") or item.get("name") or "", str(item.get("summary") or item.get("note") or ""))
            if name and name != "unknown channel" and name not in channel_names:
                channel_names.append(name)
        elif isinstance(item, str) and item.strip():
            name = _case_text(item, 80)
            if name not in channel_names:
                channel_names.append(name)
        if len(channel_names) >= 3:
            break
    if channel_names:
        return "Seen mostly in " + ", ".join(channel_names[:3]) + "."
    return "BNL does not have a specific public channel breakdown for this topic yet."
...
def _topic_buckets(memory: dict[str, Any], ownership: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    for item in candidates:
        ...
        buckets.append({"topic": topic, "strength": strength, "mainReadEligible": main_read_eligible, "ownershipBasis": ownership_basis, **({"evidenceCount": count} if count is not None else {}), "explanation": explanation, "whereSeen": _topic_where_seen(memory, topic), "exampleSignals": signals[:3] or review_signals[:2] or ["BNL detected this topic, but the current archive does not expose enough safe examples yet."]})
        if len(buckets) >= 10:
            break
    ...
    return buckets if ownership else (buckets or [{"topic": "Insufficient topic extraction", "strength": "weak", "mainReadEligible": False, "ownershipBasis": "unknown", "explanation": "The current archive does not expose enough topic detail to summarize recurring subject themes."}])
...
def _brief_signal_text(memory: dict[str, Any], topics: list[dict[str, Any]], anchors: list[dict[str, Any]], channels: list[dict[str, Any]] | None = None, ownership: dict[str, Any] | None = None) -> str:
    context = _brief_context(memory)
    if ownership:
        # Main admin read shape is allowed to see only explicitly subject-owned or subject-attached authoritative text.
...
def _subject_archetype_read(subject: str, memory: dict[str, Any], profile: dict[str, Any], topics: list[dict[str, Any]], anchors: list[dict[str, Any]], channels: list[dict[str, Any]], ownership: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if has_lore or "orion" in anchor_names:
        signals.append("Orion/interface/lore language appears in the direct subject evidence")
    if has_weird_concrete:
        signals.append("weird concrete subjects appear in direct subject evidence")
    if has_network_skeptic:
        signals.append("Network skepticism/questioning appears in direct subject evidence")
    if has_music:
    ...
    rationale = _case_text(f"BNL classifies {subject} this way because the strongest usable for the main admin read pattern is {', '.join(signals[:4]) if signals else 'too thin or generic to type safely'}. This is a shape read for internal review, not a public identity confirmation.", 520)
...
def _distinguishing_read(archetype: dict[str, Any]) -> str:
    ...
    if "orion" in signals or "interface" in signals:
        return "Distinctive because usable for the main admin read direct subject evidence is Orion/interface-linked rather than generic community chatter."
    if "bnl" in signals:
        return "Distinctive because usable for the main admin read direct subject evidence is BNL-facing rather than generic community chatter."
    ...
    if "weird concrete" in signals or "network skepticism" in signals:
        return "Distinctive because direct subject evidence includes weird concrete topics and/or Network skepticism rather than non-owned lore context."
...
def _digest_pattern_text(digest: dict[str, Any] | None) -> str:
    if not isinstance(digest, dict):
        return ""
    parts: list[str] = []
    topics = [item.get("topic") for item in digest.get("concreteTopics") or [] if isinstance(item, dict) and item.get("topic")]
    phrases = [item.get("phrase") for item in digest.get("recurringPhrases") or [] if isinstance(item, dict) and item.get("phrase")]
    behaviors = [item.get("behavior") for item in digest.get("distinctiveBehaviors") or [] if isinstance(item, dict) and item.get("behavior")]
    challenges = [item.get("pattern") for item in digest.get("questionsOrChallenges") or [] if isinstance(item, dict) and item.get("pattern")]
    moments = [item.get("moment") for item in digest.get("representativeMoments") or [] if isinstance(item, dict) and item.get("moment")]
    if topics:
        parts.append("concrete topics: " + ", ".join(topics[:4]))
    if phrases:
        parts.append("recurring phrases/terms: " + ", ".join(phrases[:3]))
    if behaviors:
        parts.append("behavior: " + ", ".join(behaviors[:3]))
    if challenges:
        parts.append("questions/challenges: " + ", ".join(challenges[:2]))
    if moments:
        parts.append("representative moment: " + moments[0])
...
_DIGEST_TAXONOMY_PHRASES = {
    "active community", "active community participant", "approved public context", "bnl facing behavior", "community context", "generic community chatter", "available archive context", "usable for the main admin read", "non-owned context", "other entities", "relationship context", "source file", "source file discussion", "shared context",
}
_DIGEST_CONCRETE_PATTERNS: list[tuple[str, str, str]] = [
    ("vibrating beds", r"\bvibrating beds?\b", "Direct subject evidence specifically mentions vibrating beds."),
    ("weird concrete topics", r"\b(weird|strange|odd|unusual)\b.{0,60}\b(topic|object|physical|concrete|bed|device|room|mattress)s?\b|\b(vibrating|mattress|physical object|strange object|odd object)s?\b", "Direct subject evidence centers on unusual concrete objects or physical subjects."),
    ("Network motive questioning", r"\b(network|barcode network|barcode)\b.{0,80}\b(why|motive|motives|intent|intentions|trust|skeptic|question|doing|what are they|what is it)\b|\b(why|motive|motives|intent|trust|skeptic|question|doing)\b.{0,80}\b(network|barcode network|barcode)\b", "Direct subject evidence questions Network/BARCODE motives or intent."),
    ("Orion/archive/AI framing", r"\b(orion)\b.{0,90}\b(archive|ai|speaks?|speaking|interface|connected intelligence)\b|\b(archive|ai|speaks?|speaking|interface|connected intelligence)\b.{0,90}\b(orion)\b", "Direct subject evidence frames Orion with archive, AI, speaking, or interface language."),
    ("archive language", r"\b(archive|archival|interface|record|records)\b", "Direct subject evidence repeats archive/interface language."),
    ("Suno links", r"\b(suno\.com|suno)\b", "Direct subject evidence includes Suno/platform link-sharing language."),
    ("posts in WIPs/demos", r"\b(wip|wips|demo|demos|finished[- ]tracks?|track|song)\b", "Direct subject evidence appears around WIP, demo, song, or finished-track language."),
    ("BNL challenge/pushback language", r"\b(bnl|bnl-01)\b.{0,80}\b(challenge|push(?:es|ed)? back|teas(?:e|es|ed)?|antagoniz|provok|argu|call(?:s|ed)? out|rules?|boundary)\b|\b(challenge|push(?:es|ed)? back|teas(?:e|es|ed)?|antagoniz|provok|argu|call(?:s|ed)? out)\b.{0,80}\b(bnl|bnl-01)\b", "Direct subject evidence shows BNL-facing challenge, teasing, or pushback."),
]
...
def _digest_owned_items(ownership: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    items: list[dict[str, Any]] = []
    for scope in _MAIN_READ_ELIGIBLE_SCOPES:
        for item in by_ownership.get(scope) or []:
            if isinstance(item, dict) and _ownership_item_scope(item) in _MAIN_READ_ELIGIBLE_SCOPES:
                items.append(item)
    for item in ownership.get("owned") or []:
        if isinstance(item, dict) and _ownership_item_scope(item) in _MAIN_READ_ELIGIBLE_SCOPES:
            items.append(item)
    seen: set[str] = set()
    out: list[dict[str, Any]] = []
    for item in items:
        text = _case_text(item.get("text"), 360)
        key = text.lower()
        if text and key not in seen:
            out.append(item)
            seen.add(key)
...
def _digest_texts(items: list[dict[str, Any]]) -> list[str]:
    return _case_list([item.get("text") for item in items], limit=120, item_limit=300)


def _digest_strength(count: int) -> str:
    return "strong" if count >= 3 else ("moderate" if count >= 2 else "weak")


def _digest_examples(evidence: list[str], limit: int = 3) -> list[str]:
    return _case_list(evidence, limit=limit, item_limit=180)


def _extract_concrete_digest_topics(texts: list[str]) -> list[dict[str, Any]]:
    topics: list[dict[str, Any]] = []
    seen: set[str] = set()
    for topic, pattern, note in _DIGEST_CONCRETE_PATTERNS:
        evidence = [text for text in texts if re.search(pattern, text, flags=re.I)]
        if not evidence:
            continue
        key = topic.lower()
        if key in seen:
            continue
        seen.add(key)
        topics.append({"topic": topic, "strength": _digest_strength(len(evidence)), "evidenceCount": len(evidence), "exampleSignals": _digest_examples(evidence), "note": note})
    # Add a few concrete noun-ish phrases from evidence when they are not generic labels.
    candidates: Counter[str] = Counter()
    examples: dict[str, list[str]] = {}
    for text in texts:
        cleaned = re.sub(r"https?://\S+", " ", text.lower())
        for match in re.finditer(r"\b(?:weird|strange|odd|unusual|vibrating|archive|network|orion|suno|wip|demo|challenge|pushback|teasing)[a-z0-9' -]{0,42}\b", cleaned):
            phrase = re.sub(r"[^a-z0-9' -]+", " ", match.group(0)).strip(" -")
            words = [w for w in phrase.split() if w and w not in _DIGEST_STOPWORDS]
            if not (1 <= len(words) <= 6):
                continue
            phrase = " ".join(words)
            if phrase in _DIGEST_TAXONOMY_PHRASES or len(phrase) < 4:
                continue
            candidates[phrase] += 1
            examples.setdefault(phrase, []).append(text)
    for phrase, count in candidates.most_common(4):
        if len(topics) >= 8:
            break
        if phrase.lower() in seen or any(phrase in str(t.get("topic", "")).lower() for t in topics):
            continue
        topics.append({"topic": phrase, "strength": _digest_strength(count), "evidenceCount": count, "exampleSignals": _digest_examples(examples.get(phrase, [])), "note": "Concrete detail extracted from evidence directly tied to this subject, not from generated topic labels."})
    return topics[:8]


def _extract_recurring_phrases(texts: list[str]) -> list[dict[str, Any]]:
    counts: Counter[str] = Counter()
    examples: dict[str, list[str]] = {}
    for text in texts:
        lowered = re.sub(r"https?://\S+", " ", text.lower())
        lowered = re.sub(r"\b\d{5,}\b|[\w.+-]+@[\w.-]+", " ", lowered)
        tokens = [tok.strip("' -") for tok in re.findall(r"[a-z][a-z0-9'-]*", lowered)]
        tokens = [tok for tok in tokens if len(tok) > 1 and tok not in _DIGEST_STOPWORDS]
        seen_in_text: set[str] = set()
        for n in range(2, 7):
            for i in range(0, max(0, len(tokens) - n + 1)):
                phrase = " ".join(tokens[i:i+n]).strip()
                if not phrase or phrase in seen_in_text or phrase in _DIGEST_TAXONOMY_PHRASES:
                    continue
                if any(word in _DIGEST_STOPWORDS for word in phrase.split()[:1] + phrase.split()[-1:]):
                    continue
                if re.search(r"\b(?:source|file|report|review|evidence|context|subject|community)\b", phrase):
                    continue
                # Prefer phrases with a distinctive anchor/term, or phrases repeated across evidence.
                distinctive = re.search(r"\b(orion|archive|ai|vibrating|beds?|network|suno|bnl|challenge|pushback|speaks?|weird|strange|motives?)\b", phrase)
                if not distinctive and n < 3:
                    continue
    ...
    for phrase, count in counts.most_common(20):
        distinctive = re.search(r"\b(orion|archive|ai|vibrating|beds?|network|suno|bnl|challenge|pushback|speaks?|weird|strange|motives?)\b", phrase)
        if count < 2 and not distinctive:
            continue
        rows.append({"phrase": _case_text(phrase, 80), "evidenceCount": count, "exampleSignals": _digest_examples(examples.get(phrase, []), 2), "note": "Repeated or distinctive phrase sampled only from evidence directly tied to this subject."})
        if len(rows) >= 8:
            break
    return rows


def _extract_digest_behaviors(texts: list[str], concrete_topics: list[dict[str, Any]]) -> list[dict[str, Any]]:
    patterns = [
        ("BNL challenge / playful antagonism", r"\b(challenge|push(?:es|ed)? back|teas(?:e|es|ed)?|antagoniz|provok|argu|call(?:s|ed)? out|tests? bnl)\b", "Owned evidence shows challenge, teasing, provocation, or pushback toward BNL."),
        ("Orion/archive speaking or AI framing", r"\b(orion|archive|ai|speaks? (?:for|through)|speaking through|connected intelligence|interface)\b", "Owned evidence frames Orion/archive/interface as an AI or speaking presence."),
        ("unusual concrete subject recurrence", r"\b(vibrating beds?|weird|strange|odd|unusual|physical|object|mattress|device)\b", "Owned evidence repeatedly centers concrete or strange physical subjects."),
        ("music/platform link sharing", r"\b(suno|spotify|soundcloud|youtube|track|song|demo|wip|finished[- ]tracks?)\b", "Owned evidence includes music/platform/link-sharing language; ownership still needs confirmation."),
    ]
    rows: list[dict[str, Any]] = []
    for behavior, pattern, note in patterns:
        evidence = [text for text in texts if re.search(pattern, text, flags=re.I)]
        if evidence:
            rows.append({"behavior": behavior, "strength": _digest_strength(len(evidence)), "exampleSignals": _digest_examples(evidence), "note": note})
    if not rows and concrete_topics:
        rows.append({"behavior": "concrete subject recurrence", "strength": concrete_topics[0].get("strength", "weak"), "exampleSignals": concrete_topics[0].get("exampleSignals", []), "note": "Behavior inferred from repeated directly tied concrete topics."})
    return rows[:8]


def _extract_questions_or_challenges(texts: list[str]) -> list[dict[str, Any]]:
    specs = [
        ("Network", r"\b(network|barcode network|barcode)\b.{0,90}\b(why|motive|motives|intent|trust|skeptic|question|doing|what are they)\b|\b(why|motive|motives|intent|trust|skeptic|question|doing)\b.{0,90}\b(network|barcode network|barcode)\b", "Network skepticism/questioning"),
        ("BNL", r"\b(bnl|bnl-01)\b.{0,90}\b(challenge|push(?:es|ed)? back|teas(?:e|es|ed)?|antagoniz|provok|argu|call(?:s|ed)? out|rules?|boundary)\b|\b(challenge|push(?:es|ed)? back|teas(?:e|es|ed)?|antagoniz|provok|argu|call(?:s|ed)? out)\b.{0,90}\b(bnl|bnl-01)\b", "BNL-facing challenge/pushback"),
        ("community", r"\b(why|question|challenge|push(?:es|ed)? back|argu|call(?:s|ed)? out)\b.{0,90}\b(community|chat|people|everyone)\b", "community-facing questioning/challenge"),
    ]
    rows: list[dict[str, Any]] = []
    for target, pattern, label in specs:
        evidence = [text for text in texts if re.search(pattern, text, flags=re.I)]
        if evidence:
            rows.append({"target": target, "pattern": label, "exampleSignals": _digest_examples(evidence), "note": "Question/challenge pattern appears in direct subject evidence."})
    return rows[:6]


def _extract_links_or_platforms(texts: list[str]) -> list[dict[str, Any]]:
    counts: Counter[str] = Counter()
    examples: dict[str, list[str]] = {}
    platform_terms = ("suno", "spotify", "soundcloud", "youtube", "bandcamp", "discord")
    for text in texts:
        lowered = text.lower()
        found: set[str] = set()
        for match in _DIGEST_LINK_RE.finditer(lowered):
    ...
    return [{"platformOrDomain": _case_text(key, 90), "evidenceCount": count, "exampleSignals": _digest_examples(examples.get(key, []), 2), "ownershipStatus": "owned", "note": "Platform/link language appears in direct subject evidence; confirm whether the linked work is owned before public claims."} for key, count in counts.most_common(8)]


def _representative_digest_moments(subject: str, texts: list[str], concrete_topics: list[dict[str, Any]], behaviors: list[dict[str, Any]], questions: list[dict[str, Any]]) -> list[dict[str, Any]]:
    body = "\n".join(texts).lower()
    rows: list[dict[str, Any]] = []
    if "orion" in body and ("archive" in body or "ai" in body or "speak" in body):
        rows.append({"moment": f"{subject} repeatedly frames Orion with archive, AI, speaking, or interface language.", "whyItMatters": "This gives admins a concrete lore/interface angle while keeping public meaning review-only."})
    if "vibrating" in body or ("network" in body and re.search(r"motive|why|skeptic|question|doing", body)):
        rows.append({"moment": f"{subject} brings up unusual concrete subjects and questions what the Network is doing.", "whyItMatters": "This supports a personality/community-pattern dossier angle rather than a generic activity label."})
    if any("challenge" in str(item.get("behavior", "")).lower() or "antagon" in str(item.get("behavior", "")).lower() for item in behaviors):
        rows.append({"moment": f"{subject} repeatedly pushes back at BNL in a challenger or antagonistic tone.", "whyItMatters": "This distinguishes the subject from generic community chatter."})
    for topic in concrete_topics[:3]:
        if len(rows) >= 6:
            break
        rows.append({"moment": f"{subject} has owned evidence around {topic.get('topic')}.", "whyItMatters": _case_text(topic.get("note") or "Concrete direct subject detail helps admins draft a stronger Source File.", 180)})
    if not rows and texts:
        rows.append({"moment": f"{subject} has limited direct subject evidence, but it does not yet form a concrete recurring pattern.", "whyItMatters": "Admin should add concrete source notes before drafting a stronger dossier."})
    return rows[:6]


def _digest_quality(texts: list[str], concrete_topics: list[dict[str, Any]], phrases: list[dict[str, Any]], behaviors: list[dict[str, Any]], questions: list[dict[str, Any]]) -> str:
    if not texts:
        return "insufficient"
    signal_score = len(concrete_topics) + len(phrases) + len(behaviors) + len(questions)
    if len(texts) >= 5 and signal_score >= 4:
        return "strong"
    if len(texts) >= 2 and signal_score >= 2:
        return "moderate"
    return "thin"


def _digest_dossier_usefulness(subject: str, quality: str, concrete_topics: list[dict[str, Any]], behaviors: list[dict[str, Any]], questions: list[dict[str, Any]], links: list[dict[str, Any]]) -> dict[str, Any]:
    body = json.dumps({"topics": concrete_topics, "behaviors": behaviors, "questions": questions, "links": links}, sort_keys=True, default=str).lower()
    useful: list[str] = []
    not_enough = ["public identity, alias certainty, or relationship meaning", "queue submissions, played tracks, payment, or Priority status"]
    next_questions = ["Which direct subject examples are public-safe enough to quote or paraphrase?", "Is the subject an artist, community participant, lore/interface figure, challenger, collaborator, or something else?"]
    if quality in {"thin", "insufficient"}:
        likely = "Not enough for a strong dossier yet. Add concrete direct source notes first."
        useful.append("Evidence triage / source-note collection")
    elif "orion" in body and ("archive" in body or "ai" in body):
        likely = "Lore/interface Source File candidate with review-only public meaning."
        useful.extend(["Orion/archive/interface framing", "community-lore behavior read"])
        next_questions.append("What does the Orion/archive language mean, and is any of it safe to say publicly?")
    elif "vibrating" in body or "weird concrete" in body or "network skepticism" in body:
        likely = "Personality/community-pattern dossier candidate."
        useful.extend(["weird concrete topics", "Network skepticism/questioning"])
        next_questions.append("Are the unusual topics one-off jokes or repeated subject-defining patterns?")
    elif "challenge" in body or "antagon" in body or "pushback" in body:
        likely = "BNL-facing challenger/community dynamics dossier candidate."
        useful.extend(["BNL challenge/pushback tone", "how the subject tests BNL boundaries"])
        next_questions.append("Is the challenger tone playful, adversarial, or context-dependent?")
    elif links:
        likely = "Music/link-sharing review candidate; ownership must be confirmed."
        useful.extend(["shared platforms/links", "possible music/community angle"])
        next_questions.append("Are the shared links owned work, references, or other people's tracks?")
    else:
        likely = "Limited community-pattern Source File candidate."
        useful.append("direct subject evidence triage")
...
def build_subject_owned_evidence_digest_v1(subject: str, ownership: dict[str, Any], profile: dict[str, Any], channels: list[dict[str, Any]], subject_authored_evidence: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    questions = _extract_questions_or_challenges(texts)
    ...
    for channel in channels[:6]:
        if not isinstance(channel, dict):
            continue
        name = _safe_channel_name(channel.get("channelName") or channel.get("channel") or "unknown channel", str(channel.get("note") or ""))
        count = _as_int(channel.get("count"))
        top_channels.append({"channelName": name, **({"count": count} if count is not None else {}), "note": _case_text(channel.get("note") or "Observed as a channel/source lane for subject evidence; verify before public use.", 180)})
    if quality in {"thin", "insufficient"} and not (concrete_topics or behaviors or moments):
        digest_read = "BNL has limited direct subject evidence. The file needs concrete source notes before a stronger dossier read is possible."
    else:
        digest_read = _case_text(f"Evidence digest for {subject}: {_digest_pattern_text({'concreteTopics': concrete_topics, 'recurringPhrases': phrases, 'distinctiveBehaviors': behaviors, 'questionsOrChallenges': questions, 'representativeMoments': moments}) or 'limited concrete detail tied to the subject'}.", 620)
    return {
        ...
        **({"latestObservedAt": profile.get("latestObservedAt")} if profile.get("latestObservedAt") else {}),
        ...
        "concreteTopics": concrete_topics,
        "recurringPhrases": phrases,
        "distinctiveBehaviors": behaviors,
        "questionsOrChallenges": questions,
        "sharedLinksOrPlatforms": links,
        "representativeMoments": moments,
        "dossierUsefulness": _digest_dossier_usefulness(subject, quality, concrete_topics, behaviors, questions, links),
    }

def _behavioral_signature_v1(subject: str, ownership: dict[str, Any], digest: dict[str, Any] | None = None) -> dict[str, Any]:
    digest = digest or {}
    owned_texts = _digest_texts(_digest_owned_items(ownership))
    body = json.dumps(digest, sort_keys=True, default=str).lower() if digest else "\n".join(owned_texts).lower()

    recurring_rows: list[dict[str, Any]] = []
    for item in digest.get("distinctiveBehaviors") or []:
        if isinstance(item, dict):
            recurring_rows.append({
                "behavior": _case_text(item.get("behavior"), 120),
                "strength": item.get("strength") if item.get("strength") in {"strong", "moderate", "weak"} else "weak",
                **({"evidenceCount": item.get("evidenceCount")} if item.get("evidenceCount") is not None else {}),
                "exampleSignals": _case_list(item.get("exampleSignals"), limit=3, item_limit=180),
                "note": _case_text(item.get("note") or "Behavior is derived from the direct subject evidence digest.", 180),
            })
    for phrase in digest.get("recurringPhrases") or []:
        if isinstance(phrase, dict) and len(recurring_rows) < 6:
            recurring_rows.append({
                "behavior": f"recurs around phrase: {_case_text(phrase.get('phrase'), 80)}",
                "strength": "moderate" if (_as_int(phrase.get("evidenceCount")) or 0) >= 2 else "weak",
                "evidenceCount": _as_int(phrase.get("evidenceCount")) or 1,
                "exampleSignals": _case_list(phrase.get("exampleSignals"), limit=2, item_limit=180),
                "note": "Phrase recurrence comes from evidence directly tied to this subject, not generated labels.",
            })

    unusual = []
    for item in digest.get("concreteTopics") or []:
        if not isinstance(item, dict):
            continue
        topic = str(item.get("topic") or "")
        if re.search(r"\b(vibrating|weird|strange|odd|unusual|concrete|physical|object|bed|mattress|device)\b", topic, flags=re.I):
            unusual.append({"subject": _case_text(topic, 120), "evidenceCount": item.get("evidenceCount"), "exampleSignals": _case_list(item.get("exampleSignals"), limit=3, item_limit=180), "note": _case_text(item.get("note") or "Unusual concrete subject appears in direct subject evidence.", 180)})

    bnl_question = next((item for item in digest.get("questionsOrChallenges") or [] if isinstance(item, dict) and item.get("target") == "BNL"), None)
    bnl_behavior = next((item for item in digest.get("distinctiveBehaviors") or [] if isinstance(item, dict) and re.search(r"bnl|challenge|antagon|pushback|push back", str(item.get("behavior") or ""), flags=re.I)), None)
    if bnl_question or bnl_behavior:
        ev = _case_list((bnl_question or bnl_behavior or {}).get("exampleSignals"), limit=4, item_limit=180)
        behavior_text = json.dumps(bnl_question or bnl_behavior, default=str).lower()
        bnl_stance = "playful_challenger" if re.search(r"teas|playful|joke|provok", behavior_text) else "antagonistic"
        bnl_conf = (bnl_behavior or {}).get("strength") if isinstance(bnl_behavior, dict) else "moderate"
        bnl_note = "Direct evidence shows challenge, antagonism, teasing, or pushback toward BNL."
        bnl_evidence = ev
    elif re.search(r"\bbnl(?:-01)?\b", body):
        bnl_stance, bnl_conf, bnl_note, bnl_evidence = "BNL-facing", "moderate", "Direct evidence references BNL without a clear challenge/support stance.", []
    else:
        bnl_stance, bnl_conf, bnl_note, bnl_evidence = "unknown", "weak", "No BNL stance from direct subject evidence is clear.", []

    net_question = next((item for item in digest.get("questionsOrChallenges") or [] if isinstance(item, dict) and item.get("target") == "Network"), None)
    if net_question:
        ev = _case_list(net_question.get("exampleSignals"), limit=4, item_limit=180)
        stance = "skeptical" if re.search(r"skeptic|doubt|trust|motive", json.dumps(net_question, default=str), flags=re.I) else "questioning"
        network = {"stance": stance, "confidence": "strong" if len(ev) >= 3 else "moderate", "evidence": ev, "note": "Direct evidence questions Network/BARCODE motives or intent."}
    elif re.search(r"\b(network|barcode|edge|lardcode)\b", body):
        network = {"stance": "lore-engaged", "confidence": "moderate", "evidence": [], "note": "Direct evidence engages Network/BARCODE language without a clear trust/skepticism stance."}
    else:
        network = {"stance": "unknown", "confidence": "weak", "evidence": [], "note": "No Network stance from direct subject evidence is clear."}

    if "orion" in body and ("archive" in body or "ai" in body):
        read = f"BNL reads {subject}'s direct evidence as Orion/archive/AI-facing; relationship meaning remains review-only unless confirmed."
    elif bnl_stance in {"antagonistic", "playful_challenger"}:
        read = f"BNL reads {subject}'s direct evidence as BNL-facing challenge or playful antagonism rather than generic community chatter."
    elif unusual or network.get("stance") in {"questioning", "skeptical"}:
        read = f"BNL reads {subject}'s direct evidence as unusual concrete-topic participation with Network skepticism/questioning when supported by the cited fragments."
    elif owned_texts:
        read = f"BNL has direct subject evidence for {subject}, but the behavioral signature is still limited and review-only."
    else:
        read = f"BNL does not have enough direct subject evidence to form a behavioral signature for {subject}."
    if not recurring_rows and owned_texts:
        recurring_rows.append({"behavior": "general direct participation", "strength": "weak", "evidenceCount": len(owned_texts), "exampleSignals": owned_texts[:2], "note": "Direct subject evidence exists but does not yet form a sharper behavioral pattern."})
    return {"signatureRead": _case_text(read, 420), "recurringBehaviors": recurring_rows[:6], "stanceTowardBNL": {"stance": bnl_stance, "confidence": bnl_conf or "weak", "evidence": bnl_evidence[:4], "note": bnl_note}, "stanceTowardNetwork": network, "unusualRecurringSubjects": unusual[:6]}
...
def _visible_disallowed_entity_terms(brief_or_context: dict[str, Any]) -> set[str]:
    ...
    def add(value: Any) -> None:
        ...
        if not text:
            return
        if text.lower() in {"bnl", "bnl-01", "when", "treat", "owned", "possible", "priority", "queue"}:
            return
        parts = [text]
        if "/" in text:
...
def _strip_non_applicable_visible_entities(text: str, disallowed_terms: set[str]) -> str:
    ...
    replacements = [
        ...
        (r"does not have direct subject evidence to treat other entities as part of this subject[’']s profile", "does not have direct subject evidence to treat other entities as part of this subject's profile"),
        ...
        (r"without other entities evidence", "without enough direct subject evidence"),
    ]
...
_VISIBLE_PRIMARY_COPY_FIELDS = {
    ...
    "confidenceRead",
    "queueSubmissionRead",
    "subjectOwnedEvidenceDigestV1",
}


_VISIBLE_JARGON_REPLACEMENTS = [
    (re.compile(r"\bsubject-owned evidence\b", re.I), "evidence tied directly to the subject"),
    (re.compile(r"\bsubject-owned\b", re.I), "directly tied to the subject"),
    (re.compile(r"\bauthored approved public-side\b", re.I), "directly participating in public BARCODE Discord"),
    (re.compile(r"\bapproved public-side discussion\b", re.I), "public BARCODE Discord conversation"),
    (re.compile(r"\bpublic-side authored item\b", re.I), "public message from the subject"),
    (re.compile(r"\bmain-read eligible\b", re.I), "usable for the main admin read"),
    (re.compile(r"\bother entities/link sharing\b", re.I), "link or relationship signals"),
    (re.compile(r"\blocal context\b", re.I), "available archive context"),
    (re.compile(r"\bthe packet cannot support\b", re.I), "BNL cannot confirm"),
    (re.compile(r"\bthe packet still cannot support\b", re.I), "BNL still cannot confirm"),
    (re.compile(r"\bSubject owned behavior note\b", re.I), "Behavior note"),
]


def _humanize_visible_source_file_text(value: str) -> str:
    text = value
    legacy_queue = "Queue/submission memory is not connected to this report. Do not claim submissions, song counts, play history, payment, or Priority status."
    plain_queue = "No confirmed queue or submission history is connected to this Source File yet. Queue/submission data is not connected."
    text = text.replace(legacy_queue, plain_queue)
    if text.strip() == "Queue/submission data is not connected.":
        text = plain_queue
    for pattern, replacement in _VISIBLE_JARGON_REPLACEMENTS:
        text = pattern.sub(replacement, text)
    text = re.sub(r"(?:No confirmed queue or submission history is connected to this Source File yet\.\s*){2,}", "No confirmed queue or submission history is connected to this Source File yet. ", text)
    return re.sub(r"\s+", " ", text).strip()


def _humanize_visible_source_file_value(value: Any) -> Any:
    if isinstance(value, str):
        return _humanize_visible_source_file_text(value)
    if isinstance(value, list):
        return [_humanize_visible_source_file_value(item) for item in value]
    if isinstance(value, dict):
        return {key: _humanize_visible_source_file_value(val) for key, val in value.items()}
    return value
...
def _scrub_visible_source_file_copy(brief: dict[str, Any], ownership: dict[str, Any], subject: str, confirmed_aliases: list[str]) -> dict[str, Any]:
        ...
        if isinstance(brief.get("topicBuckets"), list):
            for bucket in brief["topicBuckets"]:
                if isinstance(bucket, dict):
                    for key in ("explanation", "exampleSignals", "whereSeen"):
                        if key in bucket:
    ...
    for field in _VISIBLE_PRIMARY_COPY_FIELDS:
        if field in brief:
            brief[field] = _humanize_visible_source_file_value(brief[field])
    for field in ("topicBuckets", "subjectArchetypeRead"):
        if field in brief:
            brief[field] = _humanize_visible_source_file_value(brief[field])
    if isinstance(brief.get("relationshipSignals"), list):
        fixed_relationships = []
        for item in brief["relationshipSignals"]:
            text = _case_text(item, 260)
            if text.lower().strip(" .") in {"other entities", "non-owned context", "shared context"}:
                text = "BNL saw this subject appear near another name/entity, but this is not enough to confirm a relationship. Treat this as a review prompt, not a fact."
            elif not any(term in text.lower() for term in ("review", "unconfirmed", "not confirm", "uncertain")):
                text = f"{text} Treat this as a review prompt, not a fact."
            fixed_relationships.append(text)
        brief["relationshipSignals"] = fixed_relationships
    return brief
...
def _brief_ownership_routing_diagnostics(ownership: dict[str, Any], nearby_context: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in nearby_context or []:
        ...
        blocked.append({
            ...
            "routedAs": "co_mention",
            "reason": str(item.get("warning") or item.get("reason") or "non-owned context only; not direct subject evidence"),
        })
        seen.add(key)
        if len(blocked) >= 20:
            break
...
def _main_read_routing_diagnostics(ownership: dict[str, Any], topics: list[dict[str, Any]], nearby_context: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for text in _review_only_shape_texts(ownership):
        ...
        if any(term in lowered for term in ("orion", "interface", "lore")):
            add("Orion/interface/lore", "Review-only or non-owned context lacks usable for the main admin read subject ownership/explicit relationship evidence.", "nearbyContextSignals")
        elif len(excluded) < 8:
    ...
    for bucket in topics:
        ...
        if topic:
            add(topic, "Topic bucket has no usable for the main admin read owned signal; it remains review-only.", "reviewOnlyTopic")
    for item in nearby_context or []:
...
def build_subject_intelligence_brief_v1(memory: dict[str, Any]) -> dict[str, Any]:
    ...
    if not queue_connected and "not connected" not in queue_read.lower():
        queue_read = f"No confirmed queue or submission history is connected to this Source File yet. {queue_read}"
    ...
    brief = {
        "subjectRead": _build_subject_read(subject, profile, topics, channels, archetype, behavioral_signature, subject_owned_digest),
        "bnlTake": _build_bnl_take(subject, memory, topics, anchors, archetype, behavioral_signature, nearby_context, subject_owned_digest),
        ...
        "doNotSayPubliclyYet": _case_list([
            "Do not publicly call this subject an artist until admin confirms ownership.",
            "Do not claim this subject owns shared links until admin confirms link ownership.",
            "Do not connect this subject to another identity unless an alias/identity link is confirmed.",
...
def build_subject_memory_packet_v1(packet: dict[str, Any]) -> dict[str, Any]:
    ...
    queue_connected = str(packet.get("queueSubmissionStatus") or "not_connected") not in {"", "not_connected", "unknown"}
    queue_context = [
        _case_text(packet.get("queueSubmissionNote") or ("Queue/submission data is connected to this report." if queue_connected else "No confirmed queue or submission history is connected to this Source File yet. Queue/submission data is not connected."), 320)
    ]
    if queue_connected:
    ...
    if not queue_connected:
        packet_v1["evidenceGaps"].append("No confirmed queue or submission history is connected to this Source File yet. Queue/submission data is not connected.")
...
def build_source_file_case_report_v1(subject_memory_packet: dict[str, Any]) -> dict[str, Any]:
    ...
    report = {
        ...
        "queueSubmissionContext": _join_case_items(memory.get("queueSubmissionContext"), "No confirmed queue or submission history is connected to this Source File yet. Queue/submission data is not connected."),
...
def _has_existing_public_dossier_context(packet: dict[str, Any]) -> bool:
    match_kind = str(packet.get("matchKind") or "").lower()
    if match_kind in {"active_source_file", "existing_dossier_update", "public_dossier_match"}:
        return True
    source_file = packet.get("sourceFile") if isinstance(packet.get("sourceFile"), dict) else {}
    if _first(source_file, ("targetDossierId", "dossierId", "publicDossierId", "existingPublicDossierId")):
        return True
    for item in _public_dossier_context_items(packet):
        text = " ".join(_case_list([item.get(k) for k in ("subjectName", "name", "title", "slug", "subjectKey")], limit=8, item_limit=140)).lower()
        if text and (_subject_key(text) == _subject_key(packet.get("subject") or _first(source_file, ("name", "displayName", "title"))) or str(packet.get("subject") or "").lower() in text):
            return True
    return False


def _public_safe_state_items(analyst: dict[str, Any], dossier_completion_read: dict[str, Any]) -> list[str]:
    return [
        x for x in _case_list(
            analyst.get("publicReadyClaims") or analyst.get("publicSafeClaims") or dossier_completion_read.get("publicSafeToUseNow"),
            limit=8,
            item_limit=220,
        )
        if not re.search(r"\b(no public-safe claim|not confirmed|confirmed yet)\b", x, re.I)
...
def build_subject_dossier_state_v1(packet: dict[str, Any], analyst: dict[str, Any], dossier_completion_read: dict[str, Any], review_actionability: dict[str, Any] | None = None, case_report: dict[str, Any] | None = None, source_file_classification: dict[str, Any] | None = None) -> dict[str, Any]:
    """Canonical BNL-owned opinion for Source File dossier status."""

    subject = _case_text(dossier_completion_read.get("subjectName") or analyst.get("subjectName") or packet.get("subject") or "this subject", 140)
    subject_key = _subject_key(subject)
    public_items = _public_safe_state_items(analyst, dossier_completion_read)
    internal_items = _case_list([*(dossier_completion_read.get("keepInternal") or []), *(analyst.get("privateOrInternalExclusions") or []), *(analyst.get("sourceBlindInsights") or [])], limit=8, item_limit=220)
    omit_items = _case_list(dossier_completion_read.get("omitForNow") or analyst.get("doNotSayPublicly"), limit=8, item_limit=220)
    required = _case_list(dossier_completion_read.get("requiredToDraft") or analyst.get("dossierBlockedBy"), limit=8, item_limit=220)
    optional = _case_list(dossier_completion_read.get("helpfulButOptional") or analyst.get("missingInfoQuestions") or analyst.get("missingConfirmations"), limit=8, item_limit=220)
    active_items = [i for i in ((review_actionability or {}).get("items") or []) if isinstance(i, dict) and i.get("id") in set((review_actionability or {}).get("activeReviewItemIds") or [])]
    owner_needed = any(str(i.get("actionability")) == "needs_owner_wording" or str(i.get("safeDefault")) == "ask_owner" for i in active_items)
    admin_needed = any(str(i.get("actionability")) in {"required_to_finish", "needs_admin_confirmation"} for i in active_items)
    blocked = bool(any(str(i.get("actionability")) == "true_blocker" or i.get("blocksDraft") is True for i in active_items) or (not public_items and any(re.search(r"\b(name|identity|display)\b", x, re.I) for x in required)))
    existing_public = _has_existing_public_dossier_context(packet)
    candidate_new = str(packet.get("matchKind") or "").lower() in {"candidate_intake", "new_candidate", "candidate_new"}
    ready = bool((analyst.get("readyForDraft") if "readyForDraft" in analyst else dossier_completion_read.get("completionStatus") in {"ready now", "ready with omissions"}) and public_items and not blocked)
    internal_only = bool(not public_items and internal_items and not required)
    watch_only = bool(not ready and not blocked and not required and not owner_needed and not admin_needed)
    if existing_public and (ready or public_items):
        state = "update_existing_public_dossier"
        workflow = "update"
        next_action = "Open an update plan for the existing public dossier."
        mode = "update"
    elif blocked:
        state = "blocked"
        workflow = "blocked"
        next_action = "Resolve the blocking identity/public-safety decision before drafting."
        mode = "blocked"
    elif owner_needed:
        state = "needs_owner_wording"
        workflow = "ask"
        next_action = "Ask the owner for approved public wording."
        mode = "ask"
    elif admin_needed:
        state = "needs_admin_confirmation"
        workflow = "ask"
        next_action = "Ask an admin to confirm the required public-safe detail."
        mode = "ask"
    elif internal_only:
        state = "internal_only"
        workflow = "watch"
        next_action = "Keep this Source File internal and avoid public review pressure."
        mode = "watch"
    elif ready:
        state = "ready_for_cautious_draft"
        workflow = "draft"
        next_action = "Create a cautious draft using public-safe material and omissions."
        mode = "draft"
    elif candidate_new:
        state = "candidate_new"
        workflow = "build"
        next_action = "Build this new candidate into a Source File before draft work."
        mode = "build"
    elif watch_only:
        state = "watch_only"
        workflow = "watch"
        next_action = "Keep watching; refresh if new public activity appears."
        mode = "watch"
    else:
        state = "source_file_building"
        workflow = "build"
        next_action = "Build the Source File with more public-safe context."
        mode = "build"
    what_changed = _case_list(packet.get("dirtyReasons") or packet.get("refreshReasons") or packet.get("reasons") or [], limit=5, item_limit=160)
    should_refresh = bool(what_changed or str(packet.get("matchKind") or "") in {"stale_source_file", "refresh_requested"})
    return _sanitize_archive_value({
        "version": "1",
        "subjectName": subject,
        "subjectKey": subject_key,
        "state": state,
        "confidence": _case_text(analyst.get("confidence") or "medium", 40),
        "dossierAngle": _case_text(dossier_completion_read.get("likelyDossierAngle") or "community/music-adjacent dossier", 120),
        "nearestPublicDossierPattern": _case_text(dossier_completion_read.get("nearestPublicDossierPattern"), 220),
        "hasEnoughForTruthfulDossier": bool(ready or (existing_public and public_items)),
        "canDraftNow": bool(ready and not existing_public),
        "shouldRefresh": should_refresh,
        "whyNow": _case_text((dossier_completion_read.get("draftCompositionPlan") if ready else "") or next_action, 360),
        "whatChanged": what_changed,
        "whatBnlHas": _case_list(dossier_completion_read.get("whatBnlHas") or analyst.get("sourceFileIngredients"), limit=8, item_limit=220),
        "publicSafeToUseNow": public_items,
        "keepInternal": internal_items or ["Keep source-blind, queue, lore, private, and review-only context out of public copy."],
        "omitForNow": omit_items or ["Omit unresolved optional role, link, relationship, queue, lore, and collaboration details."],
        "requiredDecisions": required if state in {"blocked", "needs_owner_wording", "needs_admin_confirmation"} else [],
        "optionalEnrichments": optional,
        "linkedPublicDossiers": _sanitize_archive_value(_public_dossier_context_items(packet)[:3]),
        "recommendedWorkflow": workflow,
        "recommendedNextAction": next_action,
        "recommendedSurfaceMode": mode,
    })


def _surface_control_for_state(state: str) -> dict[str, Any]:
    mapping = {
        "ready_for_cautious_draft": ("Create cautious draft", "create_draft"),
        "update_existing_public_dossier": ("Open update plan", "open_update_plan"),
        "needs_owner_wording": ("Ask owner", "ask_owner"),
        "needs_admin_confirmation": ("Ask admin", "ask_admin"),
        "blocked": ("Resolve blocker", "ask_owner"),
        "internal_only": ("Keep internal", "keep_internal"),
        "watch_only": ("Refresh if new activity", "refresh"),
        "candidate_new": ("Build candidate", "refresh"),
        "source_file_building": ("Build Source File", "refresh"),
    }
    label, key = mapping.get(state, ("Keep watching", "refresh"))
    return {"label": label, "actionKey": key}


def build_source_file_surface_v1(subject_state: dict[str, Any], review_actionability: dict[str, Any] | None = None) -> dict[str, Any]:
    """Compact operator-facing Source File surface derived from subjectDossierStateV1."""

    subject = _case_text(subject_state.get("subjectName") or "this subject", 140)
    state = str(subject_state.get("state") or "source_file_building")
    primary = _surface_control_for_state(state)
    primary["reason"] = _case_text(subject_state.get("recommendedNextAction"), 220)
    cards: list[dict[str, Any]] = []

    def add_card(title: str, subtitle: str, control_key: str, *, reason: str = "", choices: list[str] | None = None, input_hint: str = "") -> None:
        card = {"title": _case_text(title, 90), "subtitle": _case_text(subtitle, 180), "control": {"controlType": control_key}}
        if reason:
            card["reason"] = _case_text(reason, 180)
        if choices:
            card["control"]["choices"] = choices[:5]
            card["control"]["defaultChoice"] = choices[0]
        if input_hint:
            card["control"]["inputHint"] = _case_text(input_hint, 120)
        cards.append(card)

    if state == "ready_for_cautious_draft":
        add_card("Draft from public-safe material", "BNL can fill a truthful short public dossier with safe omissions.", "create_draft", reason="Owner Review still controls publication.")
    elif state == "update_existing_public_dossier":
        add_card("Update existing public dossier", "A matching public dossier exists; prepare an update plan instead of a new draft.", "open_update_plan")
    elif state == "needs_owner_wording":
        add_card("Owner wording needed", "Only ask for the wording that unlocks the public dossier.", "ask_owner", input_hint="Owner-approved public wording or identity boundary.")
    elif state == "needs_admin_confirmation":
        add_card("Admin confirmation needed", "Confirm the required public-safe detail before drafting.", "ask_admin", input_hint="Confirmed public-safe detail.")
    elif state == "blocked":
        add_card("Public dossier blocked", "BNL does not have a safe public identity or framing yet.", "ask_owner", reason="Do not draft around an unsafe identity boundary.")
    elif state == "internal_only":
        add_card("Internal-only Source File", "Keep review pressure low; this material is not public dossier copy.", "keep_internal")
    elif state == "watch_only":
        add_card("Watch subject", "No draft action is recommended right now.", "refresh", reason="Refresh only if new public activity appears.")
    elif state == "candidate_new":
        add_card("New candidate", "Build the Source File before draft work.", "refresh")
    else:
        add_card("Build Source File", "Gather more public-safe context before draft work.", "refresh")

    for item in ((review_actionability or {}).get("items") or [])[:20]:
        if not isinstance(item, dict) or item.get("id") not in set((review_actionability or {}).get("activeReviewItemIds") or []):
            continue
        action = str(item.get("actionability") or "")
        if action == "approve_public_claim" and state not in {"needs_owner_wording", "blocked", "internal_only"}:
            add_card("Public-safe claim", _case_text(item.get("claimText"), 160), "approve_claim", reason="Approve wording; no auto-publication.", choices=["use_cautious_wording", "omit"])
        elif action in {"true_blocker", "needs_owner_wording"}:
            add_card("Decision unlocks draft", _case_text(item.get("question") or item.get("claimText"), 160), "ask_owner", choices=["ask_owner", "omit"])
        elif action in {"required_to_finish", "needs_admin_confirmation"}:
            add_card("Admin decision", _case_text(item.get("question") or item.get("claimText"), 160), "ask_admin", choices=["ask_admin", "omit"])
        if len(cards) >= 6:
            break

    questions = []
    active_ids = set((review_actionability or {}).get("activeReviewItemIds") or [])
    for q in ((review_actionability or {}).get("verificationPacketQuestions") or []):
        if isinstance(q, dict) and q.get("id") in active_ids:
            questions.append({"id": _case_text(q.get("id"), 120), "question": _case_text(q.get("question"), 220), "actionKey": _surface_control_for_state(state)["actionKey"]})
    return _sanitize_archive_value({
        "version": "1",
        "subjectName": subject,
        "headline": _case_text(f"{subject}: {state.replace('_', ' ')}", 140),
        "summary": _case_text(subject_state.get("whyNow") or subject_state.get("recommendedNextAction"), 300),
        "primaryAction": primary,
        "secondaryActions": [_surface_control_for_state("watch_only"), _surface_control_for_state("ready_for_cautious_draft")] if state not in {"blocked", "internal_only"} else [_surface_control_for_state("watch_only")],
        "sections": {
            "assessment": _case_text(subject_state.get("recommendedNextAction"), 220),
            "what_bnl_has": _case_list(subject_state.get("publicSafeToUseNow") or subject_state.get("whatBnlHas"), limit=4, item_limit=160),
            "public_material": _case_list(subject_state.get("publicSafeToUseNow"), limit=4, item_limit=160),
            "keep_internal": ["Internal/source-blind/private material is withheld from public copy."] if subject_state.get("keepInternal") else [],
            "needed_for_dossier": _case_list(subject_state.get("requiredDecisions"), limit=4, item_limit=160),
            "active_decisions": [q.get("question") for q in questions[:4]],
            "next_action": _case_text(subject_state.get("recommendedNextAction"), 180),
        },
        "activeCards": cards[:6],
        "verificationPacketQuestions": questions[:8],
