...
def render_packet_context(
    ...
    *,
    ...
    # Selected track evidence is a transcript of a specific interval. The
    ...
    for item in ordered_items:
        ...
        if (
            ...
            continue
        text = _safe_evidence_text(
            item.text, limit=len(item.text) if item in interval_items else 700,
        )
        if not text:
            continue
        supporting = tuple(
    ...
    rendered = (
        ...
        "software.\n"
        "- Derived member-profile examples support paraphrase, not exact wording "
        "or a claim that one example defines the member. This limit does not "
        "apply to separately supplied original public messages with their "
        "recorded speaker, platform and date; use those for ordinary quotations.\n"
        ...
        "- Do not reconstruct quotations from a derived gist, summary, "
        "observation or memory. Such interpretation cannot override supplied "
        "original messages or settle a dispute.\n"
